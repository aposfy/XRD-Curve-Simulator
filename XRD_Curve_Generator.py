...
#========================= A function to include Debye-Waller factor into the calculation =========================
def Debye_Waller_Factor(seita):
    DWF=30
...
Atoms_Config=open('1x1x70_H.txt','r')
Lattice_Cell=[]
...
Intensity_With_Theta=open("1x1x70_H_Result.txt","w")

#===================== A loop to go through from theta=0 to theta=180, in degrees, with steplength=0.1 degree =====================
for Theta in numpy.arange(0,90,0.1):
    Theta_In_Radians=math.radians(Theta)
    Total_Structure_Factor_At_This_Theta=0
    #===================== A loop to calculate the total structure factor of the lattice cell at a fixed 2Theta =====================
    ...
    Intensity_At_This_Theta=Intensity_Calculator(Total_Structure_Factor_At_This_Theta)*Debye_Waller_Factor(Theta_In_Radians)
